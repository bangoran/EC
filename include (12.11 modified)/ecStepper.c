...
void Stepper1_init(PinName_t pinName1, PinName_t pinName2, PinName_t pinName3, PinName_t pinName4)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		GPIO_pupd(myStepper1.pin[i], NPUPD);
		GPIO_otype(myStepper1.pin[i], PUSH_PULL);
		GPIO_ospeed(myStepper1.pin[i], FS);
	}
}
...
void Stepper2_init(PinName_t pinName1, PinName_t pinName2, PinName_t pinName3, PinName_t pinName4)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		GPIO_pupd(myStepper2.pin[i], NPUPD);
		GPIO_otype(myStepper2.pin[i], PUSH_PULL);
		GPIO_ospeed(myStepper2.pin[i], FS);
	}
}
