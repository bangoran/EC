...
// 1. Initialize GPIO port and pin as AF
	...
	GPIO_pupd(pinName,NPUPD);
//	GPIO_ospeed(pinName, HIGH_SPEED);  						// speed VHIGH=3		
...
// 2. Filter Duration (use default)
	if(ICn <3){
		TIMx->CCMR1 &= ~(0b1111 << (8*(ICn-1) + 4));
		TIMx->CCMR1 |= 0b0010 << (8*(ICn-1) + 4);
	}else{
		TIMx->CCMR2 &= ~(0b1111 << (8*(ICn-3) + 4));
		TIMx->CCMR2 |= 0b0010 << (8*(ICn-3) + 4);
	}
